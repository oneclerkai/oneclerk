...
## v22 — orb drag, Preview route, auth bg boost, box scroll, QnA badge, plus toggle (2026-05-01)
- **Orb drag**: the OneClerk OC orb on the builder canvas is now draggable anywhere inside the canvas-wrap. Mousedown/touchstart on the orb body (not the `+` button) starts a drag; orb clamps within the wrap bounds. Cursor changes to `grab`/`grabbing`.
- **Dashboard Preview route** (`route("preview")`): new full-page voice-preview screen at `#/preview`. Shows agent picker, language picker (8 options), voice picker (5 tones). An animated 64-bar waveform canvas (matching the landing page style) reacts to speaking state. Pressing "Talk to agent" speaks the agent's greeting via Web Speech API; stopping it mid-sentence is supported. An agent info card below the waveform shows the agent avatar, name, business, language, live/paused badge, and greeting quote — with an "Edit" button to jump straight into the agent builder.
- **Nav**: Preview nav item (`play-circle` icon) wired at `#/preview`, dispatched in the route dispatcher.
- **Auth background**: richer deep-dark multi-radial backdrop (`rgba(120,40,200,0.55)` purple + `rgba(255,100,40,0.45)` orange + `rgba(40,80,220,0.4)` blue over a `#120820→#0d0d18→#0a0c1a` linear gradient). Blobs enlarged to 520/480/360 px and opacity raised to 0.75; faster independent animation durations (10s/13s/9s). Grain layer re-tuned to a 22/11px dot grid. Auth modal gets a `rgba(150,100,255,0.3)` glow border + `rgba(140,60,255,0.2)` box-shadow.
- **Box body scroll**: `.agb-box-body` gets `overflow-y: auto; max-height: 130px` with a styled 4px thin scrollbar, so long content scrolls inside the card instead of overflowing.
- **QnA required badge**: `.lp-qna-required` amber pill badge injected next to the summary text of the first 3 QnA items (already enforced as non-collapsible). Items get a 3px amber left-border; flash animation (`qnaFlash`) fires when a user tries to close one.
- **Plus icon toggle + open class**: orb `+` button gets `.is-open` class (rotates 45°, turns red) while the menu is open; icon swaps to `x` (already done in v21, now the CSS `.is-open` rule reinforces it with transform + colour).
- **Flow node styles**: `.flow-node` cards get a `box-shadow`, `border-radius: 14px`, hover-lift, per-type coloured header gradients (`t-greeting/ask/branch/book/escalate/whatsapp`), larger connection handles with hover scale, and `.selected` ring.
- **Debug**: Python syntax check (`py_compile`) and Node.js `--check` both pass clean.
- Cache-bust bumped to `v=22` in `index.html`.
