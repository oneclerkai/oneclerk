...
class Conversation(Base):
    ...
    call_id: Mapped[str] = mapped_column(String, ForeignKey("calls.id", ondelete="CASCADE"), index=True, nullable=True)
    role: Mapped[str] = mapped_column(String, nullable=False)
    content: Mapped[str] = mapped_column(Text, nullable=False)
    source: Mapped[str] = mapped_column(String, default="voice")
    caller_number: Mapped[str] = mapped_column(String, nullable=True, index=True)
