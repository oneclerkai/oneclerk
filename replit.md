...
- **Landing page** (`route("auth")` in `app.js`): white paper-graph hero with circle-fading dot mesh, glassmorphic pill nav, "World's first autonomous Voice agent" headline (Poppins + pencil-textured Archivo Black "VOICE" via SVG turbulence filter `#lp-pencil`), rotating subtitle, hand-drawn dotted SVG arrows curving out to four colored sticky-note features; top-down 3D triangular light cone + floor shadow. Followed by black 3D semi-circular auto-rotating use-case carousel (`init3DCarousel`) with vertical golden glow, then white infinite-loop sticky-note testimonial track (`initInfiniteReviews`), then black footer with giant `ONECLERK` letters that arc upward (parabola) and straighten as you scroll (`initParabolaWord`).
